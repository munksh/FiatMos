...
// a new session must never mutate the old one
const beforeSessions = sq.prepare('SELECT COUNT(*) c FROM session').get().c
const beforeDetails = sq.prepare('SELECT COUNT(*) c FROM detail').get().c
const heavier = [{ name: 'Deadlift', details: [ {reps: 5, weight: 110, duration: '', note: ''} ] }]
S.saveSession(lift, rid, heavier, '')
ok('new session is added, not merged', sq.prepare('SELECT COUNT(*) c FROM session').get().c === beforeSessions + 1)
ok('old detail rows untouched', sq.prepare('SELECT COUNT(*) c FROM detail WHERE weight_kg = 105').get().c === 2)
ok('detail rows only ever grow', sq.prepare('SELECT COUNT(*) c FROM detail').get().c > beforeDetails)
ok('lastSession now returns the newest', S.lastSession(liftId, rid).components[0].details[0].weight === 110)
...
ok('session history model populated', sessModel.count === 3, sessModel.count)
